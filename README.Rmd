...
`bolero()` is similar to `dplyr::filter()` but only takes one thing, as a formula. The 
formula may be made by `mambo()` if you want to apply the same predicate to a tidyselection 
of columns: 
